...
if (strlen($_SESSION['login']) == 0) {
    ...
    if (isset($_POST['submit'])) {
        ...
        $stmt = $con->prepare("SELECT password FROM students WHERE studentRegno = ?");
        ...
        if ($num > 0) {
            ...
            if (password_verify($_POST['cpass'], $password)) {
                ...
                $stmt = $con->prepare("UPDATE students SET password = ?, updationDate = ? WHERE studentRegno = ?");
                $options = [
                    'cost' => 12,
                ];
                $password = password_hash($_POST['newpass'], PASSWORD_BCRYPT, $options);
                $stmt->bind_param("sss", $password, $currentTime, $_SESSION['login']);
                $stmt->execute();
