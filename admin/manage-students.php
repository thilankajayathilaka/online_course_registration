...
if (strlen($_SESSION['alogin']) == 0) {
    ...
    if (isset($_GET['del'])) {
        ...
        $stmt = $con->prepare("DELETE FROM students WHERE StudentRegno = ?");
    ...
    if (isset($_GET['pass'])) {
        ...
        $stmt = $con->prepare("UPDATE students SET password = ? WHERE StudentRegno = ?");
        $stmt->bind_param("ss", $newpass, $_GET['id']);
        $stmt->execute();
