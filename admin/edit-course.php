...
if (strlen($_SESSION['alogin']) == 0) {
  ...
  if (isset($_POST['submit'])) {
    ...
    $stmt = $con->prepare("UPDATE course SET courseCode = ?, courseName = ?, courseUnit = ?, noofSeats = ?, updationDate = ? WHERE id = ?");
    $stmt->bind_param("sssssi", $coursecode, $coursename, $courseunit, $seatlimit, $currentTime, $id);
    $stmt->execute();
